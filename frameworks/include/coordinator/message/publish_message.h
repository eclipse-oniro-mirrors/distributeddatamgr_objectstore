...
#ifndef PUBLISH_MESSAGE_H
#define PUBLISH_MESSAGE_H

#include "message.h"

namespace OHOS::ObjectStore {
constexpr uint32_t PUBLISH_OPERATION_LEN_LIMIT = 256 * 1024;  // 256k

struct PublishRequest final : public Message {
    PublishRequest(message_t id, std::vector<uint8_t> &&operation)
        : Message(id, MessageType::PUBLISH_REQUEST, MSG_RESPONSE_TIMEOUT), operationData(std::move(operation))
    {
        operationLen = operationData.size();
        header.dataSize = GetDataSize();
    }
    PublishRequest(const uint8_t *data, uint32_t dataSize)
    {
        if (data != nullptr) {
            uint32_t cursor = 0;
            ReadValue(data, dataSize, cursor, header);
            ReadValue(data, dataSize, cursor, operationLen);
            if (operationLen > PUBLISH_OPERATION_LEN_LIMIT) {
                LOG_ERROR("PublishRequest-%s: object operation too long", __func__);
                operationLen = PUBLISH_OPERATION_LEN_LIMIT;
            }
    ...
    virtual ~PublishRequest() = default;
    ...
    const uint8_t *GetData() override
    {
        ...
        if (msgBuffer_ == nullptr) {
            LOG_ERROR("PublishRequest-%s: Failed to create message buffer", __func__);
            return nullptr;
        }
        uint32_t cursor = 0;
