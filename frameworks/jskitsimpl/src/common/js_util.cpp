...
void TypedArray2Vector(uint8_t* data, size_t length, napi_typedarray_type type, std::vector<T>& out)
{
    ...
    switch (type) {
        ...
        case napi_biguint64_array:
            ...
            break;
        default:
            LOG_ERROR_RETURN_VOID(false, "[FATAL] invalid napi_typedarray_type!");
    }
}
...
napi_status JSUtil::GetValue(napi_env env, napi_value in, std::vector<int32_t>& out)
{
    ...
    LOG_DEBUG("array type=%{public}d length=%{public}d offset=%{public}d", (int)type, (int)length, (int)offset);
    LOG_ERROR_RETURN(status == napi_ok, "napi_get_typedarray_info failed!", napi_invalid_arg);
    LOG_ERROR_RETURN(type <= napi_int32_array, "is not int32 supported typed array!", napi_invalid_arg);
    LOG_ERROR_RETURN((length > 0) && (data != nullptr), "invalid data!", napi_invalid_arg);
...
napi_status JSUtil::SetValue(napi_env env, const std::vector<int32_t>& in, napi_value& out)
{
    ...
    LOG_ERROR_RETURN(bytes > 0, "invalid std::vector<int32_t>", napi_invalid_arg);
    void* data = nullptr;
    napi_value buffer = nullptr;
    napi_status status = napi_create_arraybuffer(env, bytes, &data, &buffer);
    LOG_ERROR_RETURN((status == napi_ok), "invalid buffer", status);
...
napi_status JSUtil::GetValue(napi_env env, napi_value in, std::vector<uint32_t>& out)
{
    ...
    LOG_ERROR_RETURN(status == napi_ok, "napi_get_typedarray_info failed!", napi_invalid_arg);
    LOG_ERROR_RETURN((type <= napi_uint16_array) || (type == napi_uint32_array), "invalid type!", napi_invalid_arg);
    LOG_ERROR_RETURN((length > 0) && (data != nullptr), "invalid data!", napi_invalid_arg);
...
napi_status JSUtil::SetValue(napi_env env, const std::vector<uint32_t>& in, napi_value& out)
{
    ...
    LOG_ERROR_RETURN(bytes > 0, "invalid std::vector<uint32_t>", napi_invalid_arg);
    void* data = nullptr;
    napi_value buffer = nullptr;
    napi_status status = napi_create_arraybuffer(env, bytes, &data, &buffer);
    LOG_ERROR_RETURN((status == napi_ok), "invalid buffer", status);
...
napi_status JSUtil::GetValue(napi_env env, napi_value in, std::vector<int64_t>& out)
{
    ...
    LOG_DEBUG("array type=%{public}d length=%{public}d offset=%{public}d", (int)type, (int)length, (int)offset);
    LOG_ERROR_RETURN(status == napi_ok, "napi_get_typedarray_info failed!", napi_invalid_arg);
    LOG_ERROR_RETURN((type <= napi_uint32_array) || (type == napi_bigint64_array), "invalid type!", napi_invalid_arg);
    LOG_ERROR_RETURN((length > 0) && (data != nullptr), "invalid data!", napi_invalid_arg);
...
napi_status JSUtil::GetValue(napi_env env, napi_value in, std::vector<double>& out)
{
    ...
    if (isTypedArray) {
        ...
        LOG_ERROR_RETURN((status == napi_ok) && isArray, "invalid data!", napi_invalid_arg);
        uint32_t length = 0;
        status = napi_get_array_length(env, in, &length);
        LOG_ERROR_RETURN((status == napi_ok) && (length > 0), "invalid data!", napi_invalid_arg);
        for (uint32_t i = 0; i < length; ++i) {
            napi_value item = nullptr;
            status = napi_get_element(env, in, i, &item);
            LOG_ERROR_RETURN((item != nullptr) && (status == napi_ok), "no element", napi_invalid_arg);
            ...
            LOG_ERROR_RETURN(status == napi_ok, "element not a double", napi_invalid_arg);
            out.push_back(vi);
        }
    }
    return status;
...
napi_status JSUtil::SetValue(napi_env env, const std::vector<double>& in, napi_value& out)
{
    ...
    (void)(in);
    (void)(out);
    LOG_ERROR_RETURN(false, "std::vector<double> to napi_value, unsupported!", napi_invalid_arg);
    return napi_invalid_arg;
}
    ...
    const napi_property_descriptor* properties, size_t count, napi_callback newcb)
{
    ...
    if (hasProp) {
        ...
        if (constructor != nullptr) {
            LOG_DEBUG("got data.distributeddata.%{public}s as constructor", propName.c_str());
            return constructor;
        }
    ...
    if (!hasProp) {
        ...
        LOG_DEBUG("save constructor to data.distributeddata.%{public}s", propName.c_str());
    }
    return constructor;
}
...
napi_ref JSUtil::NewWithRef(napi_env env, size_t argc, napi_value* argv, void** out, napi_value constructor)
{
    ...
    LOG_ERROR_RETURN(status == napi_ok, "napi_new_instance failed", nullptr);
    LOG_ERROR_RETURN(object != nullptr, "napi_new_instance failed", nullptr);

    status = napi_unwrap(env, object, out);
    LOG_ERROR_RETURN(status == napi_ok, "napi_unwrap failed", nullptr);
    LOG_ERROR_RETURN(out != nullptr, "napi_unwrap failed", nullptr);

    napi_ref ref = nullptr;
    status = napi_create_reference(env, object, 1, &ref);
    LOG_ERROR_RETURN(status == napi_ok, "napi_create_referenc!e failed", nullptr);
    LOG_ERROR_RETURN(ref != nullptr, "napi_create_referenc!e failed", nullptr);
    return ref;
}
